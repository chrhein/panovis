...
def photo_filtering(input_photo):
    ...
    ret, thresh = cv2.threshold(hed, 150, 255, cv2.THRESH_BINARY)
    ...
    canned = edge_detection(hed, False, 5)

    contours, hierarchy = cv2.findContours(hed.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    contoured_image = cv2.drawContours(image.copy(), contours, -1, (0, 0, 255), thickness=cv2.FILLED)

    cv2.imshow("Test", contoured_image)
    cv2.waitKey(0)
    exit()
